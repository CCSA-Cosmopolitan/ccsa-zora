  ...
  return (
    <div className="overflow-hidden rounded-[1.15rem] border border-zora-forest/20 bg-zora-deep text-white shadow-2xl shadow-zora-deep/20">
      <div className="zora-grid border-b border-white/10 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-xl font-semibold tracking-tight">Ask the farm, not a search box.</h2>
          ...
          <Badge className="border-white/15 bg-white/10 text-emerald-50" variant="outline">
            <span className="mr-1.5 size-1.5 rounded-full bg-emerald-400" />Reference intelligence online
          </Badge>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            value={fieldId}
          >
            {fields.map((field) => <option className="text-zora-deep" key={field.id} value={field.id}>{field.name}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="max-h-[360px] space-y-3 overflow-y-auto bg-[#08281e] p-4">
        {messages.map((message) => (
          ...
          >
            <div className="flex items-start gap-2">
              {message.role === "zora" ? <Sparkles className="mt-0.5 size-4 shrink-0 text-zora-sun" /> : null}
              <div className="min-w-0 flex-1">
                <p className={`text-sm leading-5 ${message.role === "zora" ? "text-emerald-50" : "font-medium"}`}>{message.text}</p>
                {message.result ? (
                  <div className="mt-3 border-t border-white/10 pt-3">
                    <div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-emerald-100/65">
                      ...
                      <button className="ml-auto text-zora-sun" onClick={() => speak(message.text)} type="button"><Volume2 className="size-4" /></button>
                    </div>
                    <ul className="mt-2 space-y-1.5">
                      {message.result.actions.map((action) => (
                        <li className="flex gap-2 text-xs leading-5 text-emerald-50/80" key={action}><CheckCircle2 className="mt-1 size-3 shrink-0 text-emerald-400" />{action}</li>
                      ))}
                    </ul>
                  </div>
                ) : null}
        ...
        {advisory.isPending ? <div className="w-32 animate-pulse rounded-2xl bg-white/9 p-4 text-xs text-emerald-50/60">Zora is reasoning…</div> : null}
      </div>

      <div className="border-t border-white/10 bg-zora-deep p-4">
        <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
          {prompts.map((item) => (
            <button className="shrink-0 rounded-full border border-white/15 px-3 py-1.5 text-[10px] text-emerald-50/70 hover:bg-white/10" key={item} onClick={() => void submit(item)} type="button">{item}</button>
          ))}
        </div>
        <div className="flex items-end gap-2 rounded-2xl border border-white/15 bg-white/8 p-2">
          <input
            ...
            type="file"
          />
          <label className="flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-xl text-emerald-50/70 hover:bg-white/10" htmlFor="zora-image" title="Attach crop or livestock image"><Camera className="size-4" /></label>
          <button aria-label={listening ? "Stop voice input" : "Start voice input"} className={`flex size-9 shrink-0 items-center justify-center rounded-xl ${listening ? "bg-red-500 text-white" : "text-emerald-50/70 hover:bg-white/10"}`} onClick={toggleListening} type="button">{listening ? <Square className="size-3 fill-current" /> : <Mic className="size-4" />}</button>
          <textarea
            ...
            placeholder={attachment ? `Image attached: ${attachment}` : "Ask Zora in your preferred language…"}
            value={prompt}
          />
          <Button aria-label="Send to Zora" className="size-10 shrink-0 rounded-xl bg-zora-sun text-zora-deep hover:bg-[#f2c34e]" disabled={prompt.trim().length < 2 || advisory.isPending} onClick={() => void submit()} size="icon"><Send className="size-4" /></Button>
        </div>
        <p className="mt-2 text-[10px] text-emerald-50/40">Reference guidance must be validated against field evidence and local extension protocols.</p>
      </div>
    </div>
  );
}
