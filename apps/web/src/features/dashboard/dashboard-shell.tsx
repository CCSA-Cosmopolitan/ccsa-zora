          ...
          <div className="border-t border-white/10 p-4">
            <div className="rounded-2xl border border-white/10 bg-black/10 p-3.5">
              ...
              <p className="mt-1.5 text-[11px] leading-4 text-emerald-50/55">
                {data.metrics.signedRecordCount.toLocaleString()} signed records across this FIMS
                tenant.
              </p>
            </div>
          </div>
        ...
        <main className="min-w-0">
          ...
          <div className="mx-auto max-w-[1680px] space-y-5 p-4 md:p-6">
              ...
              <div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                    A multilingual digital extension officer for farmers, researchers, institutions,
                    and climate-smart operations across Africa.
                  </p>
                </div>
                <div className="grid min-w-[330px] gap-2 sm:grid-cols-2">
                  ...
                  <BriefStat
                    label="Sensor fabric"
                    value={`${data.metrics.sensorAvailabilityPercent.toFixed(1)}%`}
                    icon={RadioTower}
                  />
            ...
            <section
              ...
              aria-label="Key agricultural metrics"
            >
              <MetricCard
                icon={Leaf}
                label="Vegetation vitality"
                value={formatNumber(data.metrics.meanNdvi, 2)}
                detail="Portfolio mean NDVI"
                trend="+4.2% this cycle"
              />
              <MetricCard
                icon={CloudRain}
                label="Climate exposure"
                value={`${data.metrics.heatRiskDays} days`}
                detail="Heat-risk outlook"
                trend="Rain likely in 24h"
                tone="sun"
              />
              <MetricCard
                icon={RadioTower}
                label="Connected agriculture"
                value={`${data.sensors.reporting}/${data.sensors.total}`}
                detail="LPWAN nodes reporting"
                trend="Network stable"
              />
              <MetricCard
                icon={UsersRound}
                label="Extension reach"
                value="1,248"
                detail="Farmers with active guidance"
                trend="+86 this month"
              />
            </section>
            ...
            {selectedField ? (
              <section className="grid gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(340px,0.85fr)]">
                <Card className="overflow-hidden border-zora-forest/15 shadow-sm">
                  <CardHeader className="flex-row items-start justify-between border-b border-border bg-white">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-zora-forest">
                        Live spatial context
                      </p>
                      <CardTitle className="mt-1">{selectedField.name}</CardTitle>
                      <CardDescription>
                        Boundary, crop vitality, evidence, and operational hotspots
                      </CardDescription>
                    </div>
              ...
              <Card>
                <CardContent className="p-8 text-center text-muted-foreground">
                  No active field is available for this organization.
                </CardContent>
              </Card>
            )}
            ...
            <section
              className="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(340px,0.7fr)]"
              id="climate"
            >
              <Card className="border-zora-forest/15">
                <CardHeader className="flex-row items-start justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Seven-day vegetation and soil-moisture intelligence
                    </CardDescription>
                  </div>
                  <div className="flex gap-3 text-[10px] font-semibold text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1">
                      <span className="size-2 rounded-full bg-zora-sun" />
                      Moisture
                    </span>
                  </div>
                </CardHeader>
            ...
            <section className="grid gap-4 lg:grid-cols-2" id="mrv">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    LPWAN, satellite, and field-device operational health
                  </CardDescription>
                </CardHeader>
              ...
              <Card>
                <CardHeader className="flex-row items-start justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Evidence completeness and climate-finance readiness
                    </CardDescription>
                  </div>
                  <Button onClick={exportMrv} size="sm" variant="outline">
                    <Download />
                    Export
                  </Button>
                </CardHeader>
                <CardContent>
                  <MrvChecklist data={data} />
                </CardContent>
              </Card>
            </section>
          </div>
...
function Brand() {
  return (
    <div className="border-b border-white/10 px-4 py-3.5">
      ...
      <p className="mt-2 px-1 text-[9px] font-semibold uppercase tracking-[0.2em] text-emerald-50/45">
...
function BriefStat({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-zora-forest/10 bg-zora-mist/80 p-3">
      <span className="flex size-9 items-center justify-center rounded-xl bg-white text-zora-forest shadow-sm">
        <Icon className="size-4" />
      </span>
      <div>
        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</p>
        <p className="text-sm font-bold text-zora-deep">{value}</p>
      </div>
    </div>
  );
}
    ...
    <Card className="border-zora-forest/10 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <CardContent className="p-4">
        <div className="flex items-start justify-between">
          ...
          <span
            className={`flex size-8 items-center justify-center rounded-xl ${tone === "sun" ? "bg-zora-sun/15 text-ochre-foreground" : "bg-zora-mist text-zora-forest"}`}
          >
            <Icon className="size-4" />
          </span>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between gap-2 text-[11px]">
          ...
          <span className="font-semibold text-field">{trend}</span>
        </div>
      </CardContent>
    </Card>
  );
    ...
    <Card className="overflow-hidden border-zora-forest/20 bg-zora-deep text-white">
      <CardHeader>
        ...
        <CardDescription className="text-emerald-50/60">
          Prioritized from field, climate, and evidence signals.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        <ActionRow
          priority="Now"
          text={
            field.condition === "water_stress"
              ? "Inspect irrigation coverage in the western zone."
              : "Validate crop condition in the latest scouting zone."
          }
        />
        <ActionRow
          priority="24h"
          text="Delay fertilizer if rainfall probability remains above 70%."
        />
...
function ActionRow({ priority, text }: { priority: string; text: string }) {
  return (
    <div className="flex gap-3 rounded-xl border border-white/10 bg-white/7 p-3">
      <span className="mt-0.5 h-fit rounded-md bg-zora-sun px-2 py-1 text-[9px] font-black uppercase tracking-wider text-zora-deep">
        {priority}
      </span>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Latest evidence-reviewed signals</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Signal
          label="NDVI"
          value={formatNumber(field.ndvi, 2)}
          percent={(field.ndvi ?? 0) * 100}
          color="bg-field"
        />
        <Signal
          label="Soil moisture"
          value={
            field.soilMoisturePercent === null
              ? "No reading"
              : `${field.soilMoisturePercent.toFixed(1)}%`
          }
          percent={field.soilMoisturePercent ?? 0}
          color="bg-zora-sun"
        />
        <div className="rounded-xl border border-terra/20 bg-terra/8 p-3">
          <div className="flex gap-2">
            <Activity className="mt-0.5 size-4 text-terra" />
            <div>
              <p className="text-xs font-semibold">Zora interpretation</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                {conditionCopy(field.condition)}
              </p>
            </div>
          </div>
        </div>
...
function KnowledgeGraphCard() {
  ...
  return (
    <Card className="overflow-hidden border-zora-forest/15" id="knowledge">
      <CardHeader className="bg-zora-mist/70">
        <div className="flex items-center gap-2 text-zora-forest">
          <BrainCircuit className="size-5" />
          <CardTitle>KGML-Ag</CardTitle>
        </div>
        <CardDescription>Knowledge graph + machine learning for agriculture</CardDescription>
      </CardHeader>
      <CardContent className="p-5">
        <div className="relative flex min-h-32 items-center justify-center">
          ...
          <div className="relative z-10 text-center">
            <p className="text-2xl font-bold text-zora-deep">8</p>
            <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-zora-forest">
              Connected domains
            </p>
          </div>
          {domains.slice(0, 4).map((domain, index) => (
            <span
              className="absolute rounded-full border border-border bg-white px-2.5 py-1 text-[10px] font-semibold shadow-sm"
              key={domain}
              style={{
                left: index % 2 ? "auto" : 0,
                right: index % 2 ? 0 : "auto",
                top: index < 2 ? 8 : "auto",
                bottom: index >= 2 ? 8 : "auto",
              }}
            >
              {domain}
            </span>
        ...
        <p className="mt-4 text-xs leading-5 text-muted-foreground">
          Transparent, context-aware recommendations grounded in CCSA research, extension guides,
          field evidence, and climate data.
        </p>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <Checklist
        ...
        value={`${data.mrv.practiceEvidenceComplete} / ${data.mrv.practiceEvidenceRequired}`}
      />
      <Checklist
        label="Sensor continuity"
        value={`${data.mrv.sensorContinuityPercent.toFixed(1)}%`}
      />
  ...
  return (
    <div className="flex items-center justify-between border-b border-border pb-2 last:border-0">
      ...
      <span className="flex items-center gap-1.5 text-xs font-semibold capitalize">
        <FileCheck2 className="size-3.5 text-field" />
        {value}
      </span>
    </div>
  );
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="flex-row items-start justify-between border-b border-border">
        <div>
          <CardTitle>FIMS farm intelligence</CardTitle>
          <CardDescription>
            Select a field to update Zora's spatial and operational context.
          </CardDescription>
        </div>
      ...
      <CardContent className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="bg-zora-mist/70 text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
              <tr>
                <th className="px-4 py-3">Field</th>
                <th className="px-4 py-3">Crop</th>
                <th className="px-4 py-3">Area</th>
                <th className="px-4 py-3">NDVI</th>
                <th className="px-4 py-3">Zora condition</th>
            ...
            <tbody>
              {fields.map((field) => (
                <tr
                  className={`cursor-pointer border-t border-border transition hover:bg-zora-mist/60 ${selectedFieldId === field.id ? "bg-secondary/45" : ""}`}
                  key={field.id}
                  onClick={() => select(field.id)}
                >
                  <td className="px-4 py-3 font-semibold text-zora-deep">{field.name}</td>
                  <td className="px-4 py-3 text-muted-foreground">{field.cropCode ?? "—"}</td>
                  <td className="px-4 py-3">{field.areaHectares.toFixed(1)} ha</td>
                  <td className="px-4 py-3 font-mono">{formatNumber(field.ndvi, 2)}</td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
                        field.condition === "healthy"
                          ? "default"
                          : field.condition === "inspect"
                            ? "warning"
                            : "alert"
                      }
                    >
                      {field.condition.replaceAll("_", " ")}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">
                    {field.lastEvidenceAt ? formatRelative(field.lastEvidenceAt) : "No evidence"}
                  </td>
                </tr>
              ))}
            </tbody>
...
function SmallMetric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-border bg-zora-mist/40 p-4">
      <p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">{label}</p>
      <p className="mt-2 text-2xl font-semibold text-zora-deep">{value}</p>
    </div>
  );
}
...
function LoadingDashboard() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-zora-deep">
      <div className="text-center">
        <Image
          ...
          width={827}
        />
        <Sprout className="mx-auto mt-4 size-6 animate-pulse text-zora-sun" />
        <p className="mt-3 text-sm font-semibold text-emerald-50">Connecting Zora intelligence…</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="max-w-md">
        <CardHeader>
          <TriangleAlert className="mb-2 size-7 text-terra" />
          <CardTitle>Intelligence connection failed</CardTitle>
...
function conditionCopy(condition: FieldSummary["condition"]) {
  if (condition === "healthy")
    return "Vegetation and moisture signals are within the expected operating range.";
  if (condition === "water_stress")
    return "Prioritize a field inspection and validate irrigation coverage within 24 hours.";
  if (condition === "inspect")
    return "A scouting review is recommended before the next input application.";
  return "Evidence is too old for a confident recommendation; schedule a field observation.";
}
