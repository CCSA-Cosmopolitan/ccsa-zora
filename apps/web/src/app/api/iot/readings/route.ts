...
function validSignature(body: string, timestamp: string, signature: string, organizationId: string) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const timestamp = request.headers.get("x-zora-timestamp") ?? request.headers.get("x-agrisense-timestamp") ?? "";
    const signature = request.headers.get("x-zora-signature") ?? request.headers.get("x-agrisense-signature") ?? "";
    const organizationId = request.headers.get("x-zora-organization") ?? request.headers.get("x-agrisense-organization") ?? "";
    const age = Math.abs(Date.now() - Date.parse(timestamp));
    if (!organizationId || Number.isNaN(age) || age > 5 * 60_000) {
      return NextResponse.json({ error: "Invalid organization or request timestamp" }, { status: 401 });
    }
