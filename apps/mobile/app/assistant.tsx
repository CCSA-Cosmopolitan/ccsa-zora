...
export default function ZoraAssistantScreen() {
  ...
  async function toggleRecording() {
    try {
      ...
      Alert.alert("Voice capture failed", error instanceof Error ? error.message : "Please try again.");
    }
  }
  ...
  async function askZora() {
    ...
    if (message.length < 2) {
      Alert.alert("Add a question", voiceUri
    ...
    setMessages((current) => [...current, { id: `${Date.now()}-farmer`, role: "farmer", text: message }]);
    setPrompt("");
    try {
      const result = await advisory.mutateAsync(message);
      setMessages((current) => [...current, { id: result.advisoryId, role: "zora", text: result.answer, result }]);
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.hero}>
          ...
          <View style={styles.heroCopy}>
            ...
            <Text style={styles.subtitle}>Voice-first, multilingual guidance grounded in field context and KGML-Ag.</Text>
          </View>
        </View>

        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.languages}>
          {languages.map((item) => (
            <Pressable key={item.id} onPress={() => setLanguage(item.id)} style={[styles.languageChip, language === item.id && styles.languageChipActive]}>
              <Text style={[styles.languageText, language === item.id && styles.languageTextActive]}>{item.label}</Text>
            </Pressable>
          ))}
        </ScrollView>
        ...
        <View style={styles.conversation}>
          {messages.map((message) => (
            <View key={message.id} style={[styles.message, message.role === "farmer" && styles.farmerMessage]}>
              <Text style={[styles.messageLabel, message.role === "farmer" && styles.farmerMessageText]}>{message.role === "zora" ? "ZORA" : "YOU"}</Text>
              <Text style={[styles.messageText, message.role === "farmer" && styles.farmerMessageText]}>{message.text}</Text>
              {message.result ? (
                <View style={styles.advisoryDetails}>
                  <Text style={styles.confidence}>{Math.round(message.result.confidence * 100)}% confidence · {message.result.severity} priority</Text>
                  {message.result.actions.map((action) => <Text key={action} style={styles.action}>• {action}</Text>)}
                  <Pressable onPress={() => Speech.speak(message.text, { language: activeLanguage.locale, rate: 0.92 })}><Text style={styles.listen}>Listen again</Text></Pressable>
                </View>
              ) : null}
            </View>
          ))}
          {advisory.isPending ? <Text style={styles.reasoning}>Zora is connecting field evidence and knowledge…</Text> : null}
        </View>
        ...
        <View style={styles.composer}>
          ...
          <View style={styles.composerActions}>
            <Pressable onPress={toggleRecording} style={[styles.micButton, recorderState.isRecording && styles.micButtonActive]}>
              <Text style={styles.micText}>{recorderState.isRecording ? "■ Stop" : "● Voice"}</Text>
            </Pressable>
            <Text style={styles.voiceStatus}>{voiceUri ? "Voice note ready" : recorderState.isRecording ? `${Math.round(recorderState.durationMillis / 1000)} sec` : ""}</Text>
            <Pressable disabled={advisory.isPending} onPress={askZora} style={styles.sendButton}><Text style={styles.sendText}>Ask Zora</Text></Pressable>
          </View>
        </View>

        <Text style={styles.disclaimer}>Reference guidance must be confirmed with field evidence and locally approved extension or veterinary protocols.</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  hero: { flexDirection: "row", gap: 14, borderRadius: 18, padding: 16, backgroundColor: "#063d2a" },
  ...
  languageChip: { borderWidth: 1, borderColor: "#c7d6cb", borderRadius: 999, paddingHorizontal: 13, paddingVertical: 8, marginRight: 8, backgroundColor: "#ffffff" },
  ...
  conversation: { gap: 9 },
  message: { alignSelf: "flex-start", maxWidth: "92%", borderRadius: 16, borderBottomLeftRadius: 5, padding: 13, backgroundColor: "#e5efe7" },
  farmerMessage: { alignSelf: "flex-end", borderBottomLeftRadius: 16, borderBottomRightRadius: 5, backgroundColor: "#063d2a" },
  ...
  advisoryDetails: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: "#b8cbbd", marginTop: 10, paddingTop: 8 },
  ...
  composer: { borderWidth: 1, borderColor: "#c7d6cb", borderRadius: 16, marginTop: 14, padding: 10, backgroundColor: "#ffffff" },
  input: { minHeight: 88, color: "#123126", fontSize: 13, lineHeight: 19 },
  composerActions: { flexDirection: "row", alignItems: "center", marginTop: 8 },
  micButton: { borderRadius: 10, paddingHorizontal: 11, paddingVertical: 9, backgroundColor: "#e5efe7" },
  ...
  sendButton: { borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, backgroundColor: "#e5aa20" },
