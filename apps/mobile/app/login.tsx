  ...
  async function signIn() {
    ...
    if (enrollment.error || !enrollment.data.totp) {
      Alert.alert(
        ...
        enrollment.error?.message ?? "Unable to start authenticator enrollment.",
      );
      return;
    }
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.card}>
        ...
        <Text style={styles.help}>
          {enrollmentUri
            ? "Open the setup link in your authenticator, then enter its six-digit code."
            : factorId
              ? "Enter your authenticator code to continue."
              : "Enter your FIMS organization workspace. Your session is encrypted in the device secure store."}
        </Text>
        {factorId ? (
          <>
            {enrollmentUri ? (
              <View style={styles.enrollment}>
                <Pressable
                  onPress={() => void Linking.openURL(enrollmentUri)}
        ...
        <Pressable
          disabled={
            submitting || (factorId ? verificationCode.length !== 6 : !email || password.length < 8)
          }
          onPress={factorId ? verifyMfa : signIn}
          style={styles.button}
        >
