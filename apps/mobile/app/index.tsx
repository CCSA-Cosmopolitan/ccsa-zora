      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.connectionBadge}>
            <View
              style={[
                ...
                network.isInternetReachable === false && styles.connectionDotOffline,
              ]}
            />
            <Text style={styles.connectionText}>
              {network.isInternetReachable === false ? "Offline" : "Online"}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.welcome}>
          ...
          <Text style={styles.welcomeText}>
            Speak in your language, show Zora a problem, or continue your offline field work.
          </Text>
          <Link href="/assistant" asChild>
            <Pressable style={styles.askButton}>
              <View style={styles.voiceOrb}>
                <Text style={styles.voiceOrbText}>●</Text>
              </View>
              <View style={styles.askCopy}>
                <Text style={styles.askTitle}>Ask Zora</Text>
                <Text style={styles.askSubtitle}>Voice + field context + KGML-Ag</Text>
              </View>
        ...
        <View style={styles.climateAlert}>
          <View style={styles.alertIcon}>
            <Text style={styles.alertIconText}>24h</Text>
          </View>
          <View style={styles.alertCopy}>
            ...
            <Text style={styles.alertText}>
              High runoff risk is expected in parts of the Abuja pilot area.
            </Text>
          </View>
        </View>
        ...
        <View style={styles.actionGrid}>
          ...
          <ActionCard
            ...
            mark="CV"
            detail="Camera evidence"
            disabled={!data.fields}
          />
          <ActionCard
            href="/scouting/new"
            label="Map a field issue"
            mark="GIS"
            detail="GPS + boundary"
            disabled={!data.fields}
          />
        ...
        <View style={styles.summaryHeader}>
          <Text style={styles.sectionTitle}>Field workspace</Text>
          <Text style={styles.sectionMeta}>FIMS connected</Text>
        </View>
        <View style={styles.summaryGrid}>
          <SummaryCard
            label="Assigned fields"
            value={String(data.fields)}
            detail="Offline field pack"
          />
          <SummaryCard
            label="Pending sync"
            value={String(data.pending)}
            detail={`${data.conflicts} conflicts`}
          />
        </View>

        <Pressable
          disabled={sync.running || network.isInternetReachable === false}
          onPress={synchronize}
          ...
          <View style={styles.syncCopy}>
            <Text style={styles.syncTitle}>
              {sync.running
                ...
                : "Synchronize offline field pack"}
            </Text>
            <Text style={styles.syncText}>
              {sync.lastError ??
                (sync.lastSyncedAt
                  ? `Last completed ${formatRelative(sync.lastSyncedAt)}. Tap to refresh.`
                  : "Download boundaries and upload queued evidence.")}
            </Text>
          </View>
        </Pressable>
        ...
        <View style={styles.summaryHeader}>
          ...
          <Text style={styles.sectionMeta}>This device</Text>
        </View>
        <View style={styles.list}>
          {data.observations.map((observation) => (
            <View key={observation.id} style={styles.row}>
              <View
                style={[
                  styles.rowIndex,
                  observation.syncStatus === "conflict" && styles.rowIndexConflict,
                ]}
              />
              <View style={styles.rowBody}>
                <Text style={styles.rowTitle}>{observation.title}</Text>
                <Text style={styles.rowMeta}>
                  {observation.fieldName} · {formatRelative(observation.observedAt)}
                </Text>
              </View>
              <Text
                style={[
                  styles.rowStatus,
                  observation.syncStatus === "synced" && styles.rowStatusSynced,
                ]}
              >
          ...
          {!data.observations.length ? (
            <Text style={styles.empty}>No scouting logs are stored on this device yet.</Text>
          ) : null}
        </View>

        {!data.fields ? (
          <Text style={styles.emptyHelp}>
            Synchronize once while connected to download your assigned FIMS field pack.
          </Text>
        ) : null}
    ...
    <Link href={href} asChild>
      <Pressable disabled={disabled} style={[styles.actionCard, disabled && styles.disabled]}>
        <View style={styles.actionMark}>
          <Text style={styles.actionMarkText}>{mark}</Text>
        </View>
